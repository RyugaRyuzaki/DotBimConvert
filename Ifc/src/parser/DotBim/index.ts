...
export class DotBim {
    ...
    async getMeshes( modelID: number, items: { [expressID: number | string]: { ids: number[], geometry: IIfcGeometry } } ) {
        for ( const expressID in items ) {
            ...
            const colTemp: Set<string> = new Set()
            for ( const colID in geometry ) {
                ...
                b += dotBimColor.b
                a += dotBimColor.a
                const colorStr = `${dotBimColor.r}-${dotBimColor.g}-${dotBimColor.b}-${dotBimColor.a}`
                colTemp.add( colorStr )
                const combined = mergeBufferGeometries( buffers.map( ( { buffer, matrix } ) => ( buffer.applyMatrix4( matrix ) ) ) )
            ...
            const element = { type, info, guid, rotation, vector, mesh_id } as IDotBimElement
            if ( geometryLength === 1 || colTemp.size === 1 ) {
                element.color = color
            } else {
                element.face_colors = face_colors
            }

            if ( this.allOption ) {
                if ( !this.meshes[expressID] ) this.meshes[expressID] = mesh
                if ( !this.elements[expressID] ) this.elements[expressID] = element
            } else {
                ...
                if ( !this.categories[type].elements[expressID] ) this.categories[type].elements[expressID] = element
            }
            newCombined.dispose()
        }
